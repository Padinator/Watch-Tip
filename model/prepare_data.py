...
def find_real_genres_to_all_user_movies(
    ...
    for user_id, reviews in users.items():
        ...
        for _, review in reviews.items():
            ...
            user_movie_histories[user_id].append(
                np.array(real_movie_genres, dtype=np.float64)
            )
            i += 1
...
def find_real_genres_to_all_user_movies_for_visualization(
    ...
    user_movie_histories = dict(
        zip(genre_names + ["username"], [[] for _ in range(len(genre_names) + 1)])
    )
    i = 0
...
if __name__ == "__main__":
    ...
    save_object_in_file(
        vars.user_history_file_path_with_real_genres, user_movie_histories
    )
    ...
    df_user_movie_histories = find_real_genres_to_all_user_movies_for_visualization(
        all_movies, all_users, all_genres
    )
    save_object_in_file(
    ...
    user_movie_histories_reduced_dim = TSNE(
        n_components=3, n_jobs=cpu_kernels
    ).fit_transform(
    ...
    usernames = list(
        load_object_from_file(vars.user_history_file_path_with_real_genres).keys()
    )
    columns_except_username = [
        col for col in df_user_movie_histories_reduced_dim if col != "username"
    ]
    user_movie_histories_reduced_dim = (
        {}
    )  # Store all dimension reduced real movie genres per user
