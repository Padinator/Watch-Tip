...
from data_preprocessing.insert_user_histories_in_db import (
    ...
    compare_strings,
    find_netflix_movie_in_database,
    match_netflix_movies_with_movies_from_database,
    do_one_matching_run,
    optimize_matchings,
)
...
netflix_movies_rk = [
    dict(
        [(f"netflix_{key}", value) if key != "netflix_id" else ("netflix_movie_id", value) for key, value in movie.items()]
    )
...
true_grouped_netflix_movies = {"The": netflix_movies[:-1], "Edge": [netflix_movies[-1]]}  # Grouped with [True]
...
class TestInsertUserHistories(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ...
            [
                ...
                "title",
                [False, True],
                {"The": movies_from_database[:-1], "Forrest": [movies_from_database[-1]]},
            ],
            [
                movies_from_database,
                "title",
                [False, False],
                {"Th": movies_from_database[:-1], "Fo": [movies_from_database[-1]]},
            ],
            ...
            [
                ...
                "original_title",
                [True],
                {"Lord": movies_from_database[:-1], "Forrest": [movies_from_database[-1]]},
            ],
            [
                movies_from_database,
                "original_title",
                [False],
                {"L": movies_from_database[:-1], "F": [movies_from_database[-1]]},
            ],
            [
                movies_from_database,
                "original_title",
                [True, True],
                {"Lordof": movies_from_database[:-1], "ForrestGump": [movies_from_database[-1]]},
            ],
            [
                movies_from_database,
                "original_title",
                [True, False],
                {"Lordo": movies_from_database[:-1], "ForrestG": [movies_from_database[-1]]},
            ],
            [
                movies_from_database,
                "original_title",
            ...
            [
                ...
                "original_title",
                [False, False],
                {"Lo": movies_from_database[:-1], "Fo": [movies_from_database[-1]]},
            ],
            # ---- Test Netflix movies ----
    ...
    def test_group_movies_by_attr_1(
        self,
        ...
        result = group_movies_by_attr(movies=movies, attr=attr, group_n_critria=grouping_criteria)
        self.assertEqual(result, expected_tokenization)

    @parameterized.expand(
        [[movies_from_database, "title", []], [movies_from_database, "original_title", []], [netflix_movies, "title", []]]
    )
    ...
    @parameterized.expand(
        [
            ...
            [
                netflix_movies[0],
                true_grouped_movies_from_database["The"],
                True,
                2,
                0.9,
                True,
                2,
                {**movies_from_database[0], **netflix_movies_rk[0]},
            ],
            [
                netflix_movies[1],
                true_grouped_movies_from_database["The"],
                ...
                True,
                2,
                {**movies_from_database[1], **netflix_movies_rk[1]},
            ],
            ...
            [
                ...
                True,
                2,
                {**movies_from_database[0], **netflix_movies_rk[2]},
            ],
            [
                netflix_movies[2],
                true_grouped_movies_from_database["The"],
    ...
    def test_find_netflix_movie_in_database_1(
        self,
        ...
        status_code, found_movie = find_netflix_movie_in_database(
            netflix_movie=netflix_movie,
            all_movies=all_movies,
            evaluate_year_difference=evaluate_year_diff,
            max_year_difference=max_year_diff,
            min_ratio=min_ratio,
    ...
    def test_match_netflix_movies_with_movies_from_database_1(
        self,
        ...
        found_movies, netflix_series, missing_movies_in_db = match_netflix_movies_with_movies_from_database(
            movies_from_database=movies_from_database,
    ...
    def test_do_one_matching_run_1(
        self,
        ...
        new_grouped_database_movies, (found_movies, netflix_series, missing_movies_in_db) = do_one_matching_run(
            grouped_database_movies=grouped_database_movies,
            ungrouped_netflix_movies=netflix_movies,
            min_ratio=0.9,
            max_year_diff=2,
    ...
    @parameterized.expand([[i / 10] for i in range(10)])
    def test_do_one_matching_run_2(self, min_ratio: float) -> None:
        ...
        new_grouped_database_movies, (found_movies, netflix_series, missing_movies_in_db) = do_one_matching_run(
            grouped_database_movies=true_grouped_movies_from_database,
            ungrouped_netflix_movies=netflix_movies,
            min_ratio=min_ratio,
            max_year_diff=2,
            group_criteria=[True, True],
        )
    ...
    def test_optimize_matchings_1(
        self, ratio_max_year_combis: List[Tuple[float, int]], group_criteria: List[Tuple[bool, bool]]
    ) -> None:
        """ """

        found_movies, netflix_series, missing_movies_in_db = optimize_matchings(
            database_movies=movies_from_database,
            netflix_movies_and_series=netflix_movies,
            ratio_max_year_combis=ratio_max_year_combis,
            group_criteria=group_criteria,
        )
        self.assertEqual(
            found_movies,
